...
export async function ocrEventImage(imageUrl, event) {
  ...
  // Download via proxy if available — Instagram CDN blocks datacenter IPs
  const proxyUrl = process.env.PROXY_URL;
  let imgRes;
  if (proxyUrl) {
    const { ProxyAgent } = await import("undici");
    imgRes = await fetch(imageUrl, {
      dispatcher: new ProxyAgent(`http://${proxyUrl}`),
      headers: { "User-Agent": "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7) CatskillsEvents/1.0" },
    });
  } else {
    imgRes = await fetch(imageUrl, {
      headers: { "User-Agent": "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7) CatskillsEvents/1.0" },
