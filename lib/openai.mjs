...
export async function resolveVenueTowns(venues) {
  ...
  const res = await client.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: `You are given a list of venue names in the Hudson Valley / Catskills region of New York State. Return a JSON object mapping each venue name to the town it is located in. If you don't know, map it to null.

Example: {"Orpheum Performing Arts Center": "Hunter", "Bearsville Theater": "Woodstock"}`,
      },
      {
        role: "user",
